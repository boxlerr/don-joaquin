...
function EntryIcon({ accion, valores_nuevos }: { accion: string; valores_nuevos: Record<string, unknown> }) {
  ...
  if (accion === "cambio_estado") {
    ...
    if (estado === "cerrado") return <CheckCircle2 size={14} className="text-slate-600" />;
    if (estado === "en_curso") return <RefreshCw size={14} className="text-blue-600" />;
    return <RefreshCw size={14} className="text-amber-600" />;
  }
  return <RefreshCw size={14} className="text-slate-500" />;
}
...
function EntryContent({ entry }: { entry: AuditTrailEntry }) {
  ...
  if (entry.accion === "crear") {
    return (
      <p className="text-sm font-semibold text-[#0F172A]">Viaje creado</p>
    );
  }
    ...
    return (
      <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="text-sm font-semibold text-[#0F172A]">Cambio de estado</span>
        ...
        {/* Observaciones */}
        {observaciones && (
          <div className="flex items-start gap-1.5 text-xs text-[#475569] bg-slate-50 rounded-md px-2 py-1.5 border border-slate-100">
  ...
  if (entry.accion === "actualizar") {
    return <p className="text-sm font-semibold text-[#0F172A]">Datos actualizados</p>;
  }
  ...
  return <p className="text-sm font-semibold text-[#0F172A]">{entry.accion}</p>;
}
...
export default function AuditTrailDrawer({ viajeId, open, onOpenChange }: AuditTrailDrawerProps) {
  ...
  return (
    ...
    >
      <div
        className="fixed right-0 top-0 h-full w-[min(480px,calc(100vw-2rem))] bg-white shadow-2xl border-l border-[#E2E8F0] flex flex-col animate-in slide-in-from-right"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#E2E8F0]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[#0F172A] font-semibold">Historial del viaje</h2>
              <p className="text-[#475569] text-xs">Todos los cambios registrados</p>
            </div>
          </div>
          <button
            onClick={() => onOpenChange(false)}
              ...
              <div className="space-y-5">
                {entries.map((entry) => (
                  <div key={entry.id} className="flex gap-4">
                    {/* Ícono en la línea */}
                    <div className="size-9 rounded-full bg-white border-2 border-[#E2E8F0] flex items-center justify-center shrink-0 z-10">
                        ...
                        {entry.usuario && (
                          <>
                            ...
                            <span className="text-[11px] text-[#475569] flex items-center gap-1">
                              <User size={10} className="text-[#0088D1]" />

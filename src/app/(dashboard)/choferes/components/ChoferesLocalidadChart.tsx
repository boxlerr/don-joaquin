...
export default function ChoferesLocalidadChart({ data }: { data: LocalidadData[] }) {
  ...
  return (
    <div className="mb-4 sm:mb-6 rounded-xl border border-border bg-card p-4 sm:p-5">
      <div className="mb-3 sm:mb-4">
        <h3 className="text-sm font-bold text-foreground">Distribución por localidad</h3>
        <p className="text-xs text-muted-foreground mt-0.5">
          Choferes activos e inactivos agrupados por localidad de residencia — tocá una barra para
          ver quiénes son
        </p>
      </div>
      {/* En celular no se achican 12 localidades a 343px sin que los nombres se
          pisen: el gráfico scrollea de costado dentro de su propio contenedor
          (nunca la página) y conserva todas las barras. */}
                ...
                content={({ active, payload }) => {
                  ...
                  return (
                    <div className="bg-popover border border-border rounded-lg px-3 py-2 shadow-md text-xs">
                      ...
                      <p className="text-primary mt-1">Ver quiénes son</p>
                    </div>
                  );
                }}
              />

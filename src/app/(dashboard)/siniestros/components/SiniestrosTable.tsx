...
export default function SiniestrosTable({
  ...
  const filtered = siniestros.filter((s) => {
    const camionLabel = s.camion
      ? `${s.camion.patente} ${s.camion.marca} ${s.camion.modelo}`.toLowerCase()
      : "";
    const choferLabel = s.chofer
      ? `${s.chofer.nombre} ${s.chofer.apellido || ""}`.toLowerCase()
      : "sin chofer / otro";
    const desc = s.descripcion.toLowerCase();
    const seg = (s.compania_seguro || "").toLowerCase();
    const query = searchTerm.toLowerCase();

    return (
      camionLabel.includes(query) ||
      choferLabel.includes(query) ||
      desc.includes(query) ||
      seg.includes(query)
    );
  });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between px-6 py-5 gap-4 bg-white border-b border-[#E2E8F0]">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <Input
              type="search"
              placeholder="Buscar patente, chofer, seguro..."
          ...
          <TableHeader className="bg-[#F8FAFC]">
            <TableRow>
              {[
                ...
                <TableHead
                  key={col}
                  className={`text-[11px] font-bold text-[#64748B] uppercase tracking-wider py-4 ${
                    col === "Fecha" ? "pl-6" : ""
                  }`}
                >
                  {col}
                  ...
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <span className="text-slate-400 text-xs font-medium mt-1">
                      {searchTerm
                        ...
                        : "Los siniestros registrados aparecerán aquí"}
                    </span>
                  </div>
                </TableCell>
              </TableRow>
                  ...
                  <React.Fragment key={s.id}>
                    ...
                    >
                      ...
                      <TableCell className="text-xs text-[#334155] font-semibold py-4 px-4">
                        {s.chofer ? (
                          `${s.chofer.nombre} ${s.chofer.apellido || ""}`
                        ) : (
                          <span className="text-slate-400 font-medium">Sin chofer / Otro</span>
                        )}
                      </TableCell>
                      <TableCell className="text-xs text-[#475569] py-4 px-4 max-w-xs truncate">
                        {s.descripcion}
                      </TableCell>
                      <TableCell className="text-xs text-[#475569] py-4 px-4">
                        {s.compania_seguro ? (
                          <div className="flex flex-col">
                            <span className="font-semibold text-slate-800">
                              {s.compania_seguro}
                            </span>
                            {s.numero_siniestro_seguro && (
                              <span className="text-[10px] text-slate-500 font-mono">
                                N° {s.numero_siniestro_seguro}
                              </span>
                            )}
                          </div>
                        ) : (
                      ...
                      <TableCell className="text-xs font-bold text-slate-900 py-4 px-4">
                        {s.monto_danos != null ? (
                          <span className="text-red-600">
                            $ {s.monto_danos.toLocaleString("es-AR")}
                          </span>
                        ) : (
                          <span className="text-slate-400">—</span>
                        )}
                      ...
                      <TableRow className="bg-[#F8FAFC]/40 hover:bg-[#F8FAFC]/40">
                        <TableCell colSpan={7} className="p-0 border-b border-[#E2E8F0]">
                          <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in-50 duration-200">
                            ...
                            {/* Detalles del Siniestro y Seguro */}

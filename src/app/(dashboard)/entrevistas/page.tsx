...
export default async function EntrevistasPage() {
  ...
  const [{ data }, { data: cvs }] = await Promise.all([
    sb
      .from("entrevistas")
      .select(
        "id, nombre, fecha_entrevista, edad, localidad, telefono, dni, email, puesto, experiencia, motivo_descarte, observaciones, preocupacional, resultado, etapa, created_at",
      )
